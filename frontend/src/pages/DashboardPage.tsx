...
export function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const personalOverdue = myTasks.filter((task) => task.member_status !== "DONE" && dayjs(task.deadline).isBefore(dayjs())).length;
    ...
    const globalOverdue = allTasks.filter((task) => task.status !== "CLOSED" && dayjs(task.deadline).isBefore(dayjs())).length;
  ...
  const urgentTasks = [...myTasks].filter((task) => task.member_status !== "DONE" && task.status !== "CLOSED").sort((a, b) => dayjs(a.deadline).valueOf() - dayjs(b.deadline).valueOf() || b.priority - a.priority).slice(0, 5);
  const activityDays = Array.from({ length: 7 }, (_, index) => activityStart.add(index, "day"));
  const completedActivity = activityDays.map((day) => {
    const tasks = myTasks.filter((task) => task.member_status === "DONE" && dayjs(task.updated_at).isSame(day, "day"));
    ...
    <AppLayout title="Tablou de bord">
      <Stack spacing={3}>
        <Stack direction={{ xs: "column", sm: "row" }} sx={{ justifyContent: "space-between", alignItems: { xs: "stretch", sm: "center" }, gap: 2 }}>
          <Box><Typography sx={{ fontSize: 30, fontWeight: 950, mb: 0.5 }}>Tablou de bord general</Typography><Typography sx={{ color: "text.secondary", fontSize: 17 }}>Privire de ansamblu asupra proiectelor, task-urilor și efortului personal.</Typography></Box>
        ...
        <Card><CardContent sx={{ p: 3 }}><Stack direction={{ xs: "column", sm: "row" }} sx={{ justifyContent: "space-between", gap: 2, mb: 2 }}><Box><Typography variant="h6">Progres personal ({stats.personalTotal} task-uri)</Typography><Typography sx={{ color: "text.secondary" }}>{stats.personalClosed} finalizate, {stats.personalInProgress} în progres, {stats.personalTodo} de făcut.</Typography>{nextDeadline ? <Typography sx={{ color: "text.secondary", mt: 0.75 }}>Următorul deadline: <Box component="span" sx={{ fontWeight: 900, color: "text.primary" }}>{nextDeadline.title}</Box>, {dayjs(nextDeadline.deadline).format("DD MMM YYYY, HH:mm")}</Typography> : null}</Box><Typography sx={{ color: accent.text, fontWeight: 950, fontSize: 30 }}>{personalProgress}%</Typography></Stack><LinearProgress variant="determinate" value={personalProgress} sx={{ height: 12, borderRadius: 99, bgcolor: "divider", "& .MuiLinearProgress-bar": { bgcolor: accent.value, borderRadius: 99 } }} /></CardContent></Card>
  ...
  return (
    <Box sx={{ overflowX: "auto" }}>
      <Box component="svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Line chart cu task-uri finalizate pe ultimele 7 zile" sx={{ width: "100%", minWidth: 560, display: "block" }}>
        ...
        {points.map((point) => {
          const tooltip = `${point.day.format("DD MMM YYYY")}: ${point.count} task-uri finalizate${point.titles.length ? ` - ${point.titles.join(", ")}` : ""}`;
          return <g key={point.day.toISOString()}><title>{tooltip}</title><circle cx={point.x} cy={point.y} r="4.5" fill="white" stroke={color} strokeWidth="2.5" /><text x={point.x} y={point.y - 9} textAnchor="middle" fill={point.count ? textColor : "currentColor"} opacity={point.count ? 0.95 : 0.45} fontSize="9" fontWeight="800">{point.count}</text><text x={point.x} y={height - 20} textAnchor="middle" fill="currentColor" opacity="0.62" fontSize="9">{point.day.format("ddd")}</text><text x={point.x} y={height - 7} textAnchor="middle" fill="currentColor" opacity="0.55" fontSize="9">{point.day.format("DD")}</text></g>;
        })}
      </Box>
