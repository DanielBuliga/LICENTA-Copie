...
type ProjectMember = { user_id: number; name?: string | null; email?: string | null; role: string; joined_at: string };
...
export function ProjectDashboardTab({ projectId, projectTitle }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    const inProgress = tasks.filter((task) => task.status === "IN_PROGRESS" || task.status === "READY_TO_CLOSE").length;
    const open = tasks.filter((task) => task.status === "OPEN").length;
    const overdue = tasks.filter((task) => dayjs(task.deadline).isBefore(dayjs()) && task.status !== "CLOSED").length;
    const progress = total ? Math.round((closed / total) * 100) : 0;
    const upcoming = [...tasks].filter((task) => task.status !== "CLOSED" && dayjs(task.deadline).isAfter(dayjs())).sort((a, b) => dayjs(a.deadline).valueOf() - dayjs(b.deadline).valueOf()).slice(0, 5);
