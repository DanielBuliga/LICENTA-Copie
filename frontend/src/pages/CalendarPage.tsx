...
export function CalendarPage() {
  ...
  function exportIcs() {
    ...
    blocks.filter((block) => !["READY_TO_CLOSE", "CLOSED"].includes(block.taskStatus)).forEach((block) => { lines.push("BEGIN:VEVENT"); lines.push(`UID:block-${block.id}@smart-planner`); lines.push(`DTSTAMP:${stamp}`); lines.push(`DTSTART:${apiDate(block.start_datetime).format("YYYYMMDDTHHmmss")}`); lines.push(`DTEND:${apiDate(block.end_datetime).format("YYYYMMDDTHHmmss")}`); lines.push(`SUMMARY:${escapeIcs(block.taskTitle)}`); lines.push(`DESCRIPTION:${escapeIcs(`${block.projectTitle} · ${block.taskStatus}`)}`); lines.push("END:VEVENT"); });
