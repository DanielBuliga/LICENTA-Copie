...
export function ProjectDashboardTab({ projectId, projectTitle }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    const overdue = leafTasks.filter((task) => apiDate(task.deadline).isBefore(dayjs()) && task.status !== "CLOSED").length;
    const progress = total ? Math.round((closed / total) * 100) : 0;
    const upcoming = [...leafTasks].filter((task) => task.status !== "CLOSED" && apiDate(task.deadline).isAfter(dayjs())).sort((a, b) => apiDate(a.deadline).valueOf() - apiDate(b.deadline).valueOf()).slice(0, 5);
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <Card><CardContent sx={{ p: 3 }}><Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 1 }}><Typography sx={{ fontWeight: 900 }}>Progres proiect</Typography><Typography variant="h4" sx={{ fontWeight: 950, color: "text.primary" }}>{stats.progress}%</Typography></Stack><Box sx={{ height: 12, bgcolor: "divider", borderRadius: 99, overflow: "hidden" }}><Box sx={{ width: `${stats.progress}%`, height: "100%", bgcolor: accent.value }} /></Box><Stack direction="row" spacing={2.5} sx={{ mt: 2, flexWrap: "wrap" }}><Typography><b>{stats.total}</b> total</Typography><Typography color="success.main"><b>{stats.closed}</b> închise</Typography><Typography color="primary.main"><b>{stats.inProgress}</b> în progres</Typography><Typography color="text.secondary"><b>{stats.open}</b> deschise</Typography></Stack></CardContent></Card>
